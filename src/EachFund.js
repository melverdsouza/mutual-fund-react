...
class EachFund extends React.Component {
  ...
  filterCategory(event) {
    let data = this.state.allFunds
      .filter(funds => funds["category"] === event.target.value)
      .splice(0, 100);
  ...
  filterFundType(event) {
    let data = this.state.allFunds
      .filter(funds => funds["fund_type"] === event.target.value)
      .splice(0, 100);
    this.setState({ filtered: data });
  }
  ...
  filterPlan(event) {
    let data = this.state.allFunds
      .filter(funds => funds["plan"] === event.target.value)
      .splice(0, 100);
    this.setState({ filtered: data });
  }

  sorting(event) {
    let data = this.state.filtered.sort((a, b) => {
        ...
        <Layout className="sections">
          <Sider className="sider">
            ...
            <Filter
              ...
              filterFundType={this.filterFundType}
            />
            <Sort sorting={this.sorting} />
          </Sider>
          <Content className="all-cards">
            {result.map(fund => (
              <EachCard fundDetails={fund} />
            ))}
          </Content>
        </Layout>
      </Layout>
